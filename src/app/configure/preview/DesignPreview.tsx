...
const DesignPreview = ({configuration}: {configuration: Configuration}) => {
    ...
    //the exclamation mark "!" is to tell type script that we know that what we are looking for exist
    const  {label: modelLabel} = MODELS.options.find(({value}) => value === model)!


//you can choose either of the code for the totalprice
    ...
    // if(finish === 'textured'){
    //     totalPrice += FINISHES.options[1].price
    // }
    ...
    const handleCheckout = () => {
        if(user){
            ...
            // need to log in
            //localStorage.setItem() is used to put something in localStorage
                ...
                <div className="bg-gray-50 p-6 sm:rounded-lg sm:p-8">
                    <div className="flow-root text-sm">
                        <div className="flex items-center justify-between py-1 mt-2">
                            ...
                            <p className="font-medium">{formatPrice(BASE_PRICE / 100)}</p>
                        </div>

                        {finish === 'textured' ? (
                            <div className="flex items-center justify-between py-1 mt-2">
                            <p className="text-gray-600">Textured finish</p>
                            {/* <p className="font-medium">{formatPrice(FINISHES.options[1].price / 100)}</p> */}
                            <p className="font-medium">{formatPrice(PRODUCT_PRICES.finish.textured / 100)}</p>
                        </div>
                        ) : null}
                            ...
                            {/* <p className="font-medium">{formatPrice(MATERIALS.options[1].price / 100)}</p> */}
                            <p className="font-medium">{formatPrice(PRODUCT_PRICES.material.polycarbonate / 100)}</p>
                        </div>
                        ) : null}
